...
export function AdminDashboard({ user, onLogout }: AdminDashboardProps) {
  ...
  const [deletingUser, setUSerToDelete] = useState<User | null>(null);
  // Categorias
  ...
  useEffect(() => {
    ...
    setConfig({
      VAT: configuracion.VAT ?? 0,
      tipSuggested: configuracion.tipSuggested ?? 0,
      printingAutomatic: configuracion.printingAutomatic ?? false,
    });
  }, [configuracion]);
  ...
  const handleSaveConfig = async () => {
    try {
      setLoading(true);
      const response = await actualizarConfiguracion(config);
      toast.success(response.message, {
        style: {
          background: "#16a34a",
          color: "#ffffff",
  ...
  return (
    <div className="h-screen bg-background flex overflow-hidden">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Switch
                                ...
                                onCheckedChange={(checked) =>
                                  actualizarDisponibilidad(item.id, checked)
                                }
                              />
                            </div>
                            <Button
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-4">
                {(
                  ...
                  const usersInRole = UserFilteres.filter(
                    (u) => u.role === mapRole(group.role),
                  );
                  return (
                    <Card key={group.role}>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="w-24">
                        <Input
                          type="number"
                          className="text-right"
                          value={config.tipSuggested}
                          onChange={(e) =>
                            setConfig({
                              ...config,
                              tipSuggested: Number(e.target.value),
                            })
                          }
                        />
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        checked={config.printingAutomatic}
                        onCheckedChange={(checked) =>
                          setConfig({ ...config, printingAutomatic: checked })
                        }
                      />
                    </div>
                  </CardContent>
      ...
      {/* CONFIRMAR ELIMINAR */}
      <AlertDialog
        open={!!productToDelete || !!deletingUser}
        onOpenChange={(open) => {
          if (!open) {
            setProductToDelete(null);
            setUSerToDelete(null);
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              El Usuario será marcado como eliminado, la cuenta será bloqueada
